...
export class AuthorityManager {
  ...
  setupClientMethods () {
    ...
    Meteor.methods({
      ...
      'ProseMeteor.latestDocState' ({ docId }) {
        ...
        return self.latestDocState({ docId, connection });
      },
  ...
  */
  removeDoc ({ docId, groupId }) {
    ...
    // removeDoc
    console.log(`---AuthorityManager authorities pre-remove for ${docId}`, Object.keys(this.authorities));
    delete this.authorities[docId];
    console.log(`---AuthorityManager authorities post-remove for ${docId}`, Object.keys(this.authorities));
  }
   ...
   */
  latestDocState ({ docId, connection }) {
    check(docId, String);
    if (!this.authorities[docId]) {
    ...
    this.authorities[docId].storeConnection({ connection });
    console.log('---latestDocState, this.authorities=', Object.keys(this.authorities));
