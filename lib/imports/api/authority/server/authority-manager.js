...
export class AuthorityManager {
  ...
  setupClientMethods () {
    ...
    Meteor.methods({
      ...
      'ProseMeteor.latestDocState' ({ docId }) {
        ...
        return self.latestDocState({ docId, connection, userId });
      },
      ...
      'ProseMeteor.stepsSince' ({ docId, version }) {
        ...
        return JSON.stringify(self.stepsSince({ docId, version, userId }));
      },
      ...
      'ProseMeteor.clientSubmitSteps' ({ docId, clientId, version, stepsJSON }) {
        ...
        const userId = Meteor.userId();
        self.receiveSteps({docId, clientId, version, userId, stepsJSON });
      },
  ...
  */
  openDoc ({ docId }) {
    ...
    const thisDoc = documentsColl.findOne({ docId });
    ...
    if (existingAuthorityOfDocId) {
      // TODO! run health check against this server. if it fails, remove it from registry
    ...
    for (let i = 0; i < eligibleAuthorities.length; i++) {
      ...
      if (authority.fullUrl === this.registry.fullUrl) {
        ...
        // if this server is chosen as the server hosting that doc, we can create a local authority
        this.createAuthority({ docId, groupId });
  ...
  */
  instructServerToCreateAuthority ({ fullUrl, docId, groupId = null }) {
    ...
    try {
      ...
      throw new Meteor.Error('authority-ddp-connection-failed', `ProseMeteor AuthorityManager: ddp connection to chosen authority server failed: ${e}`);
    }
  }

  /**
  * Removes a document. Removes the doc from its Authority server in the registry, and
  * deletes the document's Authority from memory on this server.
  * @param {Object} params
  * @param {String} params.docId         the unique id of the doc
  * @param {String} [params.groupId]     an id to associate this doc with other docs
  */
  removeDoc ({ docId, groupId }) {
    if (typeof this.authorities[docId] === 'undefined') {
      throw new Meteor.Error('no-authority-exists', `removeDoc: This AuthorityManager doesn't have an Authority tracking a document with docId ${docId}.`);
    }
    this.registry.removeDocFromAuthorityServer({ docId, groupId });
    // removeDoc
  ...
  * @param {String}     params.userId       userId of requesting user
  */
  stepsSince ({ version, docId, userId }) {
    check(version, Number);
    check(docId, String);
    if (!this.authorities[docId]) {
      throw new Meteor.Error('no-authority-exists', `stepsSince: This AuthorityManager doesn't have an Authority tracking a document with docId ${docId}.`);
    }
    return this.authorities[docId].stepsSince({ version, userId });
  }
  ...
  */
  receiveSteps ({ docId, version, stepsJSON, clientId, userId }) {
    check(docId, String);
    check(version, Number);
    check(stepsJSON, Array);
    check(clientId, Number);
    if (!this.authorities[docId]) {
      throw new Meteor.Error('no-authority-exists', `receiveSteps: This AuthorityManager doesn't have an Authority tracking a document with docId ${docId}.`);
    }
    this.authorities[docId].receiveSteps({ clientId, version, stepsJSON, userId });
