...
export class Authority {
  ...
  */
  ...
  startDocActivityTimeoutTimer () {
    console.log(`---authority startDocActivityTimeoutTimer() for ${this.docId}`);
    this.stopDocActivityTimeoutTimer();
    this.docActivityTimeoutTimerId = Meteor.setTimeout(() => {
      this.streamerServer.emit('authorityDocActivityTimeout', { docId: this.docId });
      this.cleanUpAndRemove();
  ...
  stopDocActivityTimeoutTimer () {
    if (this.docActivityTimeoutTimerId) {
      console.log(`---stopDocActivityTimeoutTimer() for ${this.docId} found existing interval, stopping`);
      Meteor.clearTimeout(this.docActivityTimeoutTimerId);
    } else {
      console.log(`---stopDocActivityTimeoutTimer() for ${this.docId} no existing interval`);
    }
  }
