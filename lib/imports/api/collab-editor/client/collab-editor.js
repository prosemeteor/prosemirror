...
export default class CollabEditor {
  ...
  */
  createEditor ({ version, docJSON }) {
    ...
    this.streamerClient.on('authorityDocActivityTimeout', this.handleDocActivityTimeout);
  }
  ...
  handleDocActivityTimeout ({ docId }) {
    console.log('---client received docActivityTimeout event, docId=', docId);
    if (docId !== this.docId) {
      console.log(`--client ignoring docActivityTimeout because docId in event ${docId} !== ${this.docId}`);
      // this editor only cares about its own doc
      return;
    }
    console.log('--calling this.onDocActivityTimeout, this.onDocActivityTimeout = ', this.onDocActivityTimeout);
    this.onDocActivityTimeout({ docId });
  }
}
