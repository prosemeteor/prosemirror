...
export const removeDocFromAuthorityServer = new ValidatedMethod({
  ...
  run ({ docId, fullUrl, groupId = null }) {
    let result = {
      groupIdRemoved: false
    };
    // first remove this docId
    authorityRegistryColl.update({ fullUrl }, { $pull: { docIds: docId } });
    if (groupId) {
      const thisAuthority = authorityRegistryColl.findOne({ fullUrl }, { fields: { docIds: 1, groupIds: 1 } });
      let thisAuthoritiesDocuments = documentsColl.find({ docId: { $in: thisAuthority.docIds } }).fetch();
      let numberDocsRemainingInGroupId = 0;
      thisAuthoritiesDocuments.forEach((doc) => {
        if (doc.groupId === groupId) {
          numberDocsRemainingInGroupId++;
        }
      });
      // if the removed doc was last in its group in this Authority, remove the groupId as well
      if (numberDocsRemainingInGroupId === 0) {
        authorityRegistryColl.update({ fullUrl }, { $pull: { groupIds: groupId } });
        result.groupIdRemoved = true;
      }
      return result;
    }
  }
